...
router.patch('/perfil', authMiddleware, [
  ...
  body('regime_tributario').optional().isIn(['Simples Nacional', 'Lucro Presumido', 'Lucro Real']),
  body('telefone').optional().trim(),
  body('email_contato').optional().isEmail().normalizeEmail(),
  body('email_comercial').optional().isEmail().normalizeEmail(),
